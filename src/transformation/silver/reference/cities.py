...
def process_cities_run(
    ...
    """
    Loads one run of the cities bronze table and processes it to silver history.
    """
    ...
    invalid_df = staged_df.filter(
        ...
        F.col("city_name").isNull() | (F.col("city_name") == "")
    )
    ...
    valid_df = (
        ...
        .filter(
            ...
            F.col("city_name").isNotNull() & (F.col("city_name") != "")
        )
    ...
    quarantine_df = (
        ...
        .withColumn(
            ...
            ).otherwise(F.lit("city_name missing or invalid"))
        )
        .select(
            "city_code_raw",
            "country_code_raw",
    ...
    compare_df = (
        ...
        .select(
            "src.city_code",
            "src.country_code",
            F.col("src.city_name").alias("new_city_name"),
            ...
            F.col("cur.city_name").alias("current_city_name"),
            F.col("cur.utc_offset").alias("current_utc_offset"),
            F.col("cur.time_zone_id").alias("current_time_zone_id"),
        )
    )

    new_df = compare_df.filter(F.col("current_city_name").isNull())

    unchanged_df = compare_df.filter(
        F.col("current_city_name").isNotNull() &
        F.col("new_city_name").eqNullSafe(F.col("current_city_name")) &
    ...
    if unchanged_updates_df.take(1):
        (
            ...
            .merge(
                ...
                "t.city_code = s.city_code AND t.country_code = s.country_code AND t.is_current = true"
            )
    ...
    inserts_df = (
        new_df.select(
            "city_code",
            "country_code",
            F.col("new_city_name").alias("city_name"),
...
def load_cities_to_silver(
    ...
    """
    Orchestrate the full Bronze-to-Silver cities load.
    """
    entity_name = "cities_history"
